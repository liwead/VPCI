...
IR_files = sorted(get_img_file(r"/home/liuyong/dataset/MSRS-main/train/ir"))
VIS_files   = sorted(get_img_file(r"/home/liuyong/dataset/MSRS-main/train/vi"))

assert len(IR_files) == len(VIS_files)
h5f_dir = './data'
if not os.path.exists(h5f_dir):
    os.makedirs(h5f_dir)
h5f = h5py.File(os.path.join('./data',
                                 data_name+"_"+str(img_size)+"_"+str(stride)+'.h5'), 
                    'w')
...
with h5py.File(os.path.join('data',
                                 data_name+"_"+str(img_size)+"_"+str(stride)+'.h5'),"r") as f:
    for key in f.keys():
